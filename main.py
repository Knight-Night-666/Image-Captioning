...
def save_model(model, num_epochs,embed_sz,dataset,enc_dim,dec_dim):
    ...
    torch.save(model_state, 'attention.pth')
...
def train(epochs,print_each,model,criterion,optimizer,dls,vocab_sz,validation_loader,dataset,device):
    ...
    for epoch in range(1, epochs+1):
        ...
        save_model(model, epoch, embed_sz=model.embed_sz,dataset=dataset,enc_dim=model.enc_dim,dec_dim=model.dec_dim)
    ...
    for idxx, (batch_images,batch_captions,batch_image_ids) in enumerate(iter(val_loader)):
        with torch.no_grad():
            ...
            for i in range(batch_size):
                ...
                image_pred_caption,_ = model.decoder.generate_caption(image_feature, vocab=dataset.vocab)
...
def calc_meteor_score(y_true,y_pred):
    ...
    for i in range(len(y_true)):
        m_score+=meteor_score(y_true[i],y_pred[i])

    print('Meteor score: ', m_score/len(y_true))
